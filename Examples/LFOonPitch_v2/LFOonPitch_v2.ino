...
void setup() {
  ...
  Osc::triGen(triTable); // fill wavetable
...
//    Serial.println(pitch);
  ...
  if (msNow > freqTime) {
    freqTime += 26; // update time (in milliseconds) is used above to set LFO freq 
    // Compute the LFO value to modulate the pitch (freqency) by
    // = osc val / osc range * depth + 1 
   float lfo1Val = LFO1.atTime(msNow) / (float)MAX_16 * 0.05 + 1; 
    // update the oscillator pitch (freqency)
    aOsc1.setFreq(freq1 * lfo1Val);
  }
  // For ESP32 include a call to audioUpdate in the main loop() to keep I2S buffer supplied.
  audioUpdate();
}
